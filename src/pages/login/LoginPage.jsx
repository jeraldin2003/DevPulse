      ...
      <div className="flex items-center gap-2">
        {[1, 2, 3].map((s) => (
          <div key={s} className="flex items-center gap-2 flex-1">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${forgotStep >= s ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-400'
              }`}>{s}</div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative flex items-center">
              ...
              <input
                ...
                autoComplete="new-password"
                className={`w-full pl-10 pr-4 py-2 border rounded-lg text-sm outline-none focus:ring-1 text-slate-700 placeholder:text-slate-400 disabled:opacity-60 transition-colors ${fpConfirmPassword && fpConfirmPassword !== fpNewPassword
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200/40">
            {['login', 'register'].map((m) => (
              <button
                key={m}
                type="button"
                className={`flex-1 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 cursor-pointer capitalize ${mode === m ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                  }`}
